...
const MENU_ITEMS = [
  ...
  { id: "team", labelId: "Tim", labelEn: "Team" },
...
export default function NavbarV2() {
  ...
  const linkClass = (id) =>
    `relative text-sm font-medium transition-colors py-1 px-3 rounded-lg hover:bg-slate-50 transition ${
      activeSection === id
        ? "text-primary"
                ...
                <ul className="space-y-0.5">
                  {MENU_ITEMS.map((item) => (
                    <li key={item.id}>
                      <button
                        onClick={() => handleNav(item.id)}
                        className={`w-full text-left block px-4 py-3 rounded-xl text-sm font-medium transition-colors cursor-pointer ${
